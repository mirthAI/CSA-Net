...
# Multi-Layer Perceptron (MLP) module definitio
class Mlp(nn.Module):
...
class Embeddings(nn.Module):
    ...
    def __init__(self, config, img_size, in_channels=3):
        ...
        if self.hybrid:
            ...
            self.hybrid_model_prev = ResNetV2(block_units=config.resnet.num_layers, width_factor=config.resnet.width_factor)
            self.hybrid_model_next = ResNetV2(block_units=config.resnet.num_layers, width_factor=config.resnet.width_factor)
...
# Decoder block for the segmentation head
...
class VisionTransformer(nn.Module):
    ...
    def forward(self, x_prev,x,x_next):
        ...
        x, attn_weights, features = self.transformer(x_prev,x,x_next)  # (B, n_patch, hidden)
